...
Browser tabs connect over the host app's WebSocket route (see ws.serve_tab) and
register a send callable here. The ui_* MCP tools (tools.py) call
`hub.request(...)` on the app's BridgeHub instance, which sends a frame to the
target tab and awaits its reply as an asyncio future. Long-running invocations
get a handle record that accumulates progress/done/error events for `ui_wait`
polling. All state is in-memory; handles are TTL-pruned an hour after
completion.
"""
