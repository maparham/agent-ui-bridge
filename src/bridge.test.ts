...
import { handleFrame, startAgentBridge } from "./bridge.js";
import { registerAction, clearRegistryForTest } from "./registry.js";
...
describe("bridge frame handling", () => {
  ...
  it("confirm-kind action: acks a handle, then done on approve", async () => {
    const { requestAgentConfirm } = await import("./confirm.js");
  ...
  it("confirm-kind action: confirmContext is shown in the dialog but not passed to the handler", async () => {
    ...
    expect(seen).toEqual({ symbol: "ABC" });
  ...
  it("confirm-kind action: confirmWarning reaches the dialog and not the handler", async () => {
    ...
    const c = collector();
    await handleFrame({ id: "13", op: "invoke", action: "warn_action", args: { symbol: "ABC" } }, c.send);
    await new Promise((r) => setTimeout(r, 0));
  ...
  it("abort dismisses a pending confirm dialog: a late approve cannot run the handler", async () => {
    ...
    const { requestAgentConfirm } = await import("./confirm.js");
    const actual = await vi.importActual<typeof import("./confirm.js")>("./confirm.js");
  ...
  it("readOnly frame runs a read-kind action normally", async () => {
    registerAction({
      name: "report.result", description: "d", kind: "read",
      params: { type: "object", properties: {} },
      handler: async () => ({ pnl: 1 }),
    });
    const c = collector();
    await handleFrame(
      { id: "14", op: "invoke", action: "report.result", args: {}, readOnly: true },
      c.send,
    );
